<script lang="ts">
  ...
  $: opened = $page.url.pathname === href || $page.url.pathname.split('/')[1].includes(href.slice(1));
</script>

{#if parent}
<li>
  <button on:click={() => (opened = !opened)} class="flex relative w-full h-fit py-3 text-xl shadow-sm z-10 shadow-cscol-500">
