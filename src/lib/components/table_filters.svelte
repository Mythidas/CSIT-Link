...
<div class={`flex flex-col ${filters_open ? "w-80" : " w-11"} space-y-2 p-2 transition-[width] bg-base-150 stroke-font border-r-2 border-accent-100`}>
  <div class="flex space-x-2">
    ...
    {#if filters_open}
    <div class="w-full">
      <Input value={null} placeholder="Search filters..."/>
    </div>
    {/if}
  </div>
  {#if filters_open}
  <div class="flex flex-col border-t-2 border-base-300">
    ...
    <div class={`${group.expanded && "pb-2"} border-b-2 border-base-300`}>
      ...
      <div class="pl-2 space-y-1">
        ...
        <div class="pl-6">
          {#if filter.type === "Text"}
          <Input bind:value={filter.value} placeholder={`Enter ${filter.name}...`} on:change={on_filter_change} />
          {/if}
        </div>
        {/if}
        {/each}
