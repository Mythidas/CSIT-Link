...
export async function delete_device_av(client: PoolClient, device_id: number, cookies: Cookies) {
  try {
    ...
    if (av_status.meta.status !== 200) {
      console.log(`[delete_device_av] Failed to delete AV device ${device.hostname} | ${device.av_id}: ${av_status.meta.error.error}`);
      return undefined;
    }
  ...
  try {
    ...
    for (let i = 0; i < pre_devices.length; i++) {
      ...
      try {
        ...
        if (av_device_res.device_list) {
          ...
          if (_device_av) {
            await client.query(update_av_query, [
              ...
              _device_av.tamper === undefined ? "NULL" : String(_device_av.tamper),
              _device_av.health,
        ...
        if (rmm_device_res.device_list) {
          ...
          if (_device_rmm) {
            await client.query(update_rmm_query, [
              ...
              _device_rmm.firewall === undefined ? "NULL" : String(_device_rmm.firewall),
              _device_rmm.uac === undefined ? "NULL" : String(_device_rmm.uac),
        ...
        console.log(`[load_devices] Failed to Update Devices: ${err}`);
      }
    }
