...
export async function load({ locals, cookies }) {
  try {
    let old_sophos_devices: _SophosDeviceEXT[] = [];
    ...
    for await (const site of db_sites) {
      ...
      for (const _device of _devices) {
        if (new Time(_device.lastSeenAt).is_older_than_30_days()) {
          old_sophos_devices.push({ ..._device, site_id: site.site_id, title: site.title });
        }
      }
    }
